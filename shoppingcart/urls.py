...
urlpatterns = [
    ...
    re_path(
        r"^account/login_session/$",
        ensure_csrf_cookie_cross_domain(login.LoginSessionView.as_view()),
    ...
    re_path(
        r"^accounts/get-user/(?P<username>[\w.+-]+)", views.GetUserAccountView.as_view(), name="get_user_account_api"
    ),
    ...
    re_path(
        r"^analytics/enrollment/batch", views.GetBatchEnrollmentDataView.as_view(), name="get_batch_enrollment_data"
    ),
]
